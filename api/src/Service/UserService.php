...
class UserService
{
    ...
    private function employeeUserExistsWithUsername(array $user, array $employee): ?array
    {
        ...
        $existingUsers = $this->commonGroundService->getResourceList(['component' => 'uc', 'type' => 'users'], ['username' => $user['username']], false)['hydra:member'];
        if (count($existingUsers) > 0) {
            $existingUser = $existingUsers[0];
            //If a user exists with the email, check if an employee exists with this user.
            $existingEmployees = $this->commonGroundService->getResourceList(['component' => 'gateway', 'type' => 'employees'], ['person.user.username' => $existingUser['username']]); // add to query?: 'person._uri' => $existingUser['person']
            if (count($existingEmployees) > 0) {
                ...
                $this->commonGroundService->deleteResource(['component' => 'gateway', 'type' => 'employees', 'id' => $employee['id']]);
                ...
                $this->mailService->sendWelcomeMail($user, 'Welkom bij TOP');
